...
class _DocumentConverter(documents.element_visitor(args=1)):
    ...
    def visit_run(self, run, context):
        ...
        if run.is_highlight:
            paths.append(self._find_style_for_run_property("highlight", default="mark"))
        if run.font_size != None:
            paths.append(self._find_style_for_run_property("font_size", default='r class="font' + str(int(run.font_size)) + '"'))
...
def _document_matcher_matches(matcher, element, element_type):
    if matcher.element_type in ["underline", "strikethrough", "all_caps", "small_caps", "bold", "italic", "highlight", "comment_reference"]:
